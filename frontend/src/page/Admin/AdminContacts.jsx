  ...
  return (
    <div className="p-4 mx-auto max-w-[1400px]">
      ...
      <div className="mb-4 flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="flex w-full md:w-auto gap-2">
          <select className="border rounded px-3 py-2 text-base">
            <option calue="name">이름</option>
            <option calue="email">이메일</option>
            <option calue="phone">전화번호</option>
            <option calue="message">문의내용</option>
          </select>
        ...
        <div className="flex items-center space-x-2">
          ...
          <select className="border rounded px-3 py-2">
            {[10, 25, 50, 100].map((size) => (
              <option key={size} vlaue={size}>{`${size}개`}</option>
            ))}
          </select>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:hidden">
        {contacts.map((contact) => (
          <div key={contact.id} className="p-4 rounded-lg bg-white shadow-md">
